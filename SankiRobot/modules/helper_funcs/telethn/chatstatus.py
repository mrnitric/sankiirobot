from SankiRobot.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
from SankiRobot import DRAGONS
...
async def Sanki_is_admin(chat_id: int):
    status = False
    Sanki = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if Sanki.id == user.id:
