...
# ported to Sanki @Mr_dark_prince
from SankiRobot import OWNER_ID, BOT_ID
from SankiRobot import telethn as tbot
import SankiRobot.modules.sql.aihelp_sql as sql
import SankiRobot.modules.sql.chatbot_sql as ly
...
string = (
  "ɪ ʙᴇʟᴏɴɢ ᴛᴏ @Sanki_Papa !",
  "ɪ ᴀᴍ ғᴀɪʀʟʏ ʏᴏᴜɴɢ ᴀɴᴅ ᴍᴀᴅᴇ ʙʏ @Sanki_Papa !",
)
...
async def _(event):
  ...
  if "Thergiakis" in result:
   ...
   pro = "Yeah, My name is ‣ 𝙎𝙖𝙣𝙠𝙞 𝙍𝙤𝙗𝙤𝙩 [🇮🇳]"
   try:
      async with tbot.action(event.chat_id, 'typing'):
           await event.reply(pro)
   except CFError as e:
